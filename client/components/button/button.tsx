...
export class Button extends React.PureComponent<ButtonProps, {}> {
  ...
  render() {
    const { className, waiting, label, primary, accent, dangerous, plain, hidden, active, big, round, tooltip, children, ...props } = this.props;
    const btnProps = props as Partial<ButtonProps>;
    if (hidden) return null;

    btnProps.className = cssNames('Button', className, {
      waiting, primary, accent, plain, active, big, round, dangerous
    });
