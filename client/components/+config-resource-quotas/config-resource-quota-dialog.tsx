import "./config-resource-quota-dialog.scss";
...
import {IResourceQuotaValues, ResourceQuota, resourceQuotaApi} from "../../api/endpoints/resource-quota.api";
...
@observer
export class ConfigResourceQuotaDialog extends React.Component<Props> {
  ...
  @observable static Data: ResourceQuota = null;
  @observable quotaName = "";
  @observable quotaSelectValue = "";
  @observable quotaInputValue = "";
  @observable namespace = this.defaultNamespace;
  @observable quotas = ConfigResourceQuotaDialog.defaultQuotas;

  static open(object: ResourceQuota) {
    ConfigResourceQuotaDialog.isOpen = true;
    ConfigResourceQuotaDialog.Data = object;
  }

  get resourceQuota() {
    return ConfigResourceQuotaDialog.Data
  }

  static close() {
    ConfigResourceQuotaDialog.isOpen = false;
  }

  onOpen = async () => {
    this.quotaName = this.resourceQuota.getName();
    this.namespace = this.resourceQuota.getNs();
    Object.entries(this.resourceQuota.spec.hard).find(([quota, value]) => {
      this.quotas[quota] = value
    });
  }
  ...
  close = () => {
    ConfigResourceQuotaDialog.close();
  }
  ...
  reset = () => {
    ...
    this.quotas = ConfigResourceQuotaDialog.defaultQuotas;
  }
  ...
  render() {
    ...
    const header = <h5><Trans>Update ResourceQuota</Trans></h5>;
    return (
      <Dialog
        {...dialogProps}
        className="ConfigResourceQuotaDialog"
        isOpen={ConfigResourceQuotaDialog.isOpen}
        onOpen={this.onOpen}
        close={this.close}
      >
        <Wizard header={header} done={this.close}>
          <WizardStep
            ...
            nextLabel={<Trans>Update</Trans>}
            next={this.updateQuota}
          >
